...
public class ServiceManager : MonoBehaviour {
	...
	void Start () {
        ...
        _Services.Add(new UIServiceProvider(this));
    ...
    void _ProcessSendData()
	{
		...
		foreach (var s in _Services) {
			...
			if (data.Length == 0)
            {
                //Debug.Log(s.GetName() + " data length 0");
                continue;
            }
...
//		_style.font.material.color = Color.white;
    ...
    public void TorchRange(float barValue)
    {
        var s = GetService(UIServiceProvider.ServiceName) as UIServiceProvider;
    ...
    public void SetTorchColor(float r, float g, float b, float a)
    {
        var s = GetService(UIServiceProvider.ServiceName) as UIServiceProvider;
